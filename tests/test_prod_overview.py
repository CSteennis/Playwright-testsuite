...
@pytest.fixture
def before_each_test(page: Page, set_testid):
    Home(page).navigate()

#AC1
def test_product_overview(page: Page, before_each_test: None):
    expect(page.get_by_role('list').filter(has=page.get_by_test_id('product-1'))).to_be_visible()

#AC2
def test_product_card_information(page: Page, before_each_test: None):
    grid = page.get_by_role('list').filter(has=page.get_by_test_id('product-1'))
